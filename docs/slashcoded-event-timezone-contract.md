...
SlashCoded event observers must include timezone metadata with each uploaded event. Local API stores UTC timestamps as authoritative event time, and it stores timezone metadata as the local context where the event happened.
...
Every observer should send these top-level event fields when uploading to Local API:
...
- `timezoneSource` should be `producer` for values captured directly by this observer. The field value remains `producer` for ingestion compatibility.
- `occurredAt` remains UTC-authoritative for ordering and dedupe.
- Observers should not put timezone only inside nested payload JSON when they can send top-level fields.
...
Browser, VSCode, and Node/Electron-style observers should use the platform `Intl` API:
...
## Windows .NET Observer

The Windows observer should report both canonical IANA data and Windows compatibility data when possible.
...
Observer tests should verify:

- Every uploaded event includes `timezoneOffsetMinutes` and `timezoneSource`.
- JS observers include an IANA `timezone` from `Intl.DateTimeFormat().resolvedOptions().timeZone`.
- The Windows observer includes `windowsTimezone` and includes canonical `timezone` when conversion succeeds.
...
Local API will accept events without timezone metadata during migration, but observers should start sending these fields as soon as they adopt this contract.
